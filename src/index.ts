...
export async function activate(context: ExtensionContext): Promise<void> {
	...
	subscriptions.push(
		...
		commands.registerCommand("languagetool.check.document", async () => {}),
		...
		commands.registerCommand("languagetoo.configure", async () => {
			// const serverUrl = await workspace.requestInput('Enter the LanguageTool server URL');
			// const language = await workspace.requestInput('Enter the language to use for checking');
			// workspace.getConfiguration().update('LanguageTool.serverUrl', serverUrl, true);
			// workspace.getConfiguration().update('LanguageTool.language', language, true);
			// languageClient.sendNotification('LanguageToolConfigurationChanged', {
			//     serverUrl,
			//     language,
			// });
		}),
